...
function capitalizeNames(names: string[]): string[] {//פונקציה בשם קפיטלליזניימ מקבלת מערך שמות ומחזירה מערך שמות
    ...
    for (let i = 0; i < names.length; i++) {//  לולאה שאומרת אינדקס =0 ומערך לפי אורך המערך ואז מוסיפים אחד בכל פעם שהלולאה רצה על האינדקס הנכון
        ...
        // const firstLetter = element[0].toUpperCase()// "D" 
        // const restName = element.slice(1, element.length) // "aniel"
        // capitalLetters.push(firstLetter + restName);//שם של המערך השני מוסיפים לו פעולת פוש למערך החדש
        const newLetter = element.replace(element[0], element[0].toUpperCase())
        capitalLetters.push(newLetter)
    }
